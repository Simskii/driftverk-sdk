...
2. Review the package license before its first public release.
3. From this checkout, run `npm ci`, `npm login`, then `npm publish --access public`. Complete npm's authentication prompt. This creates version 0.1.0.
4. In the npm package settings, add a GitHub Actions trusted publisher with these exact values:
   - Organization or user: `Simskii`
   - Repository: `driftverk-sdk`
   - Workflow filename: `publish.yml`
   - Environment: leave empty
   - Allowed action: `npm publish`
5. Set the GitHub repository Actions variable `NPM_PUBLISH_ENABLED` to `true`.
6. Update the README and the website SDK installation guide once the npm package is available.
